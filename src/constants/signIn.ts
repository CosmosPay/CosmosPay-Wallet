/**
 * The wallet's own sign-in — Google, GitHub or an emailed code. Data only; the protocol is
 * `src/lib/signIn.ts` and the backup it restores is `src/lib/cloudBackup.ts`.
 *
 * The numbers here are the wallet's half of a contract whose other half is the dev
 * platform's wallet-auth-core module (a separate repository). Where both name the same fact
 * the server is authoritative and this file stays inside it.
 */

/** The providers the platform can run. Which of them a deployment OFFERS comes from
 *  `GET /api/wallet/auth/providers` — this list is only what the wallet knows how to show. */
export const SIGN_IN_PROVIDERS = ['google', 'github'] as const;
...
/** What a deployment offers, from `GET /api/wallet/auth/providers`. */
 ...
 * under the platform's ten-minute handshake lifetime, so the wallet stops with copy it
 ...
 * exists before any vault does, and it is worthless on its own — the platform hands the
