...
export const T: Record<string, Record<Lang, string>> = {
  ...
  'recovery.error.discovery': { es: '{server} no publica un stellar.toml válido (clave de firma, red y endpoint de recuperación).', en: '{server} does not publish a valid stellar.toml (signing key, network and recovery endpoint).', pt: '{server} não publica um stellar.toml válido (chave de assinatura, rede e endpoint de recuperação).', de: '{server} veröffentlicht keine gültige stellar.toml (Signaturschlüssel, Netzwerk und Wiederherstellungs-Endpunkt).', fr: '{server} ne publie pas de stellar.toml valide (clé de signature, réseau et point d’accès de récupération).' },
  'recovery.error.noIdentityRoute': { es: 'Estos servidores de recuperación no pueden verificar tu correo por este camino. Iniciá sesión con Cosmos Pay para recuperar la cuenta.', en: 'These recovery servers cannot verify your email this way. Sign in with Cosmos Pay to recover the account.', pt: 'Estes servidores de recuperação não conseguem verificar o seu email por este caminho. Inicie sessão com a Cosmos Pay para recuperar a conta.', de: 'Diese Wiederherstellungsserver können Ihre E-Mail auf diesem Weg nicht prüfen. Melden Sie sich mit Cosmos Pay an, um das Konto wiederherzustellen.', fr: 'Ces serveurs de récupération ne peuvent pas vérifier votre e-mail de cette façon. Connectez-vous avec Cosmos Pay pour récupérer le compte.' },
  ...
  'recover.codesTitle': { es: 'Dos códigos, uno por servidor', en: 'Two codes, one from each server', pt: 'Dois códigos, um de cada servidor', de: 'Zwei Codes, einer von jedem Server', fr: 'Deux codes, un par serveur' },
  'recover.codesDesc': { es: 'Cada servidor de recuperación le envió su propio código a {email}. Ingresá los dos: ninguno de los dos acepta la palabra del otro.', en: 'Each recovery server sent its own code to {email}. Enter both: neither server takes the other’s word for it.', pt: 'Cada servidor de recuperação enviou o seu próprio código para {email}. Introduza os dois: nenhum servidor aceita a palavra do outro.', de: 'Jeder Wiederherstellungsserver hat einen eigenen Code an {email} gesendet. Geben Sie beide ein: Kein Server verlässt sich auf das Wort des anderen.', fr: 'Chaque serveur de récupération a envoyé son propre code à {email}. Saisissez les deux : aucun serveur ne se fie à la parole de l’autre.' },
  'recover.codeLabel': { es: 'Código del servidor {role}', en: 'Code from server {role}', pt: 'Código do servidor {role}', de: 'Code von Server {role}', fr: 'Code du serveur {role}' },
  'recover.codesCta': { es: 'Verificar códigos', en: 'Verify codes', pt: 'Verificar códigos', de: 'Codes prüfen', fr: 'Vérifier les codes' },
  'recover.codeInvalid': { es: 'El código del servidor {role} no es correcto. Te quedan {n} intentos.', en: 'The code from server {role} is not right. {n} attempts left.', pt: 'O código do servidor {role} não está correto. Restam {n} tentativas.', de: 'Der Code von Server {role} stimmt nicht. Noch {n} Versuche.', fr: 'Le code du serveur {role} est incorrect. Il reste {n} tentatives.' },
  'recover.codeExpired': { es: 'El código del servidor {role} venció o se bloqueó. Empezá de nuevo.', en: 'The code from server {role} expired or was locked. Start again.', pt: 'O código do servidor {role} expirou ou foi bloqueado. Comece de novo.', de: 'Der Code von Server {role} ist abgelaufen oder gesperrt. Beginnen Sie von vorn.', fr: 'Le code du serveur {role} a expiré ou a été bloqué. Recommencez.' },
  'recover.proofExpired': { es: 'La verificación de tu correo venció. Volvé a iniciar sesión para recuperar la cuenta.', en: 'The proof of your email expired. Sign in again to recover the account.', pt: 'A verificação do seu email expirou. Inicie sessão de novo para recuperar a conta.', de: 'Der Nachweis Ihrer E-Mail ist abgelaufen. Melden Sie sich erneut an, um das Konto wiederherzustellen.', fr: 'La preuve de votre e-mail a expiré. Reconnectez-vous pour récupérer le compte.' },
