...
export const T: Record<string, Record<Lang, string>> = {
  ...
  /* The two recovery servers, in developer mode. */
  'settings.epRecoveryA': { es: 'Servidor de recuperación A', en: 'Recovery server A', pt: 'Servidor de recuperação A', de: 'Wiederherstellungsserver A', fr: 'Serveur de récupération A' },
  'settings.epRecoveryB': { es: 'Servidor de recuperación B', en: 'Recovery server B', pt: 'Servidor de recuperação B', de: 'Wiederherstellungsserver B', fr: 'Serveur de récupération B' },
  ...
  'guard.notCommissionWallet': { es: 'La operación envía dinero a una cuenta que no es la tuya ni la comisión que mostró la cotización ({destination}). Firma rechazada.', en: 'The operation sends money to an account that is neither yours nor the commission the quote showed ({destination}). Signature refused.', pt: 'A operação envia dinheiro para uma conta que não é a tua nem a comissão que a cotação mostrou ({destination}). Assinatura recusada.', de: 'Die Operation sendet Geld an ein Konto, das weder deines noch die im Angebot gezeigte Provision ist ({destination}). Signatur abgelehnt.', fr: 'L’opération envoie de l’argent vers un compte qui n’est ni le tien ni la commission indiquée par le devis ({destination}). Signature refusée.' },
  'guard.overCommission': { es: 'La comisión de la transacción supera la que mostró la cotización ({amount} {code}). Firma rechazada.', en: 'The commission in this transaction is higher than the quote showed ({amount} {code}). Signature refused.', pt: 'A comissão da transação é superior à que a cotação mostrou ({amount} {code}). Assinatura recusada.', de: 'Die Provision der Transaktion ist höher als im Angebot gezeigt ({amount} {code}). Signatur abgelehnt.', fr: 'La commission de la transaction dépasse celle indiquée par le devis ({amount} {code}). Signature refusée.' },
  'guard.commissionAsset': { es: 'La comisión se cobra en un activo distinto al de la cotización ({moved}, esperado {expected}). Firma rechazada.', en: 'The commission is charged in a different asset than the quote ({moved}, expected {expected}). Signature refused.', pt: 'A comissão é cobrada num ativo diferente do da cotação ({moved}, esperado {expected}). Assinatura recusada.', de: 'Die Provision wird in einem anderen Asset als im Angebot berechnet ({moved}, erwartet {expected}). Signatur abgelehnt.', fr: 'La commission est prélevée dans un actif différent de celui du devis ({moved}, attendu {expected}). Signature refusée.' },
  'guard.commissionUnreadable': { es: 'La wallet no puede leer la comisión que declaró la cotización. Firma rechazada.', en: 'The wallet cannot read the commission the quote declared. Signature refused.', pt: 'A wallet não consegue ler a comissão que a cotação declarou. Assinatura recusada.', de: 'Die Wallet kann die im Angebot angegebene Provision nicht lesen. Signatur abgelehnt.', fr: 'Le portefeuille ne peut pas lire la commission déclarée par le devis. Signature refusée.' },
  ...
  /* SEP-30 recovery setup — the template refusals. Each one is a way an envelope can look
     like a recovery setup and be an account takeover; see `assertRecoveryTemplate`. */
  'guard.recoveryOps': { es: 'La transacción no tiene la forma de una alta de recuperación ({count} operaciones en lugar de {expected}). Firma rechazada.', en: 'This transaction is not shaped like a recovery setup ({count} operations instead of {expected}). Signature refused.', pt: 'A transação não tem a forma de uma ativação de recuperação ({count} operações em vez de {expected}). Assinatura recusada.', de: 'Diese Transaktion hat nicht die Form einer Wiederherstellungs-Einrichtung ({count} Operationen statt {expected}). Signatur abgelehnt.', fr: 'Cette transaction n’a pas la forme d’une activation de récupération ({count} opérations au lieu de {expected}). Signature refusée.' },
  'guard.recoveryPresigned': { es: 'La transacción ya viene firmada por alguien más ({count} firmas). No es la que te mostramos. Firma rechazada.', en: 'This transaction already carries someone else’s signatures ({count}). It is not the one you were shown. Signature refused.', pt: 'A transação já vem assinada por outra pessoa ({count} assinaturas). Não é a que lhe mostrámos. Assinatura recusada.', de: 'Diese Transaktion trägt bereits fremde Signaturen ({count}). Sie ist nicht die gezeigte. Signatur abgelehnt.', fr: 'Cette transaction porte déjà les signatures de quelqu’un d’autre ({count}). Ce n’est pas celle qui vous a été montrée. Signature refusée.' },
  'guard.recoverySameSigner': { es: 'Los dos servidores de recuperación declararon la misma clave. Serían uno solo. Firma rechazada.', en: 'Both recovery servers declared the same key. That would be one server, not two. Signature refused.', pt: 'Os dois servidores de recuperação declararam a mesma chave. Seriam um só. Assinatura recusada.', de: 'Beide Wiederherstellungsserver haben denselben Schlüssel genannt. Das wäre einer statt zwei. Signatur abgelehnt.', fr: 'Les deux serveurs de récupération ont déclaré la même clé. Cela ferait un seul serveur. Signature refusée.' },
  'guard.recoverySelfSigner': { es: 'Un servidor de recuperación declaró tu propia cuenta como firmante. Firma rechazada.', en: 'A recovery server declared your own account as its signer. Signature refused.', pt: 'Um servidor de recuperação declarou a sua própria conta como signatário. Assinatura recusada.', de: 'Ein Wiederherstellungsserver hat Ihr eigenes Konto als Unterzeichner angegeben. Signatur abgelehnt.', fr: 'Un serveur de récupération a déclaré votre propre compte comme signataire. Signature refusée.' },
  'guard.recoverySelfSponsor': { es: 'La transacción dice que tu cuenta se patrocina a sí misma. Firma rechazada.', en: 'This transaction says your account is sponsoring itself. Signature refused.', pt: 'A transação diz que a sua conta se patrocina a si própria. Assinatura recusada.', de: 'Diese Transaktion besagt, dass Ihr Konto sich selbst sponsert. Signatur abgelehnt.', fr: 'Cette transaction indique que votre compte se parraine lui-même. Signature refusée.' },
  'guard.recoveryNoSponsorship': { es: 'Se esperaba el patrocinio de las reservas y la transacción no lo trae. Firma rechazada.', en: 'The reserve sponsorship was expected and this transaction does not carry it. Signature refused.', pt: 'Esperava-se o patrocínio das reservas e a transação não o traz. Assinatura recusada.', de: 'Die Reserve-Übernahme wurde erwartet, diese Transaktion enthält sie nicht. Signatur abgelehnt.', fr: 'Le parrainage des réserves était attendu et cette transaction ne le contient pas. Signature refusée.' },
  'guard.recoverySponsorSource': { es: 'Las reservas las paga una cuenta distinta de la que te mostramos ({source}). Firma rechazada.', en: 'The reserves are being paid by an account other than the one you were shown ({source}). Signature refused.', pt: 'As reservas são pagas por uma conta diferente da que lhe mostrámos ({source}). Assinatura recusada.', de: 'Die Reserven zahlt ein anderes als das gezeigte Konto ({source}). Signatur abgelehnt.', fr: 'Les réserves sont payées par un compte autre que celui qui vous a été montré ({source}). Signature refusée.' },
  'guard.recoverySponsored': { es: 'El patrocinio no es para tu cuenta sino para {account}. Firma rechazada.', en: 'The sponsorship is not for your account but for {account}. Signature refused.', pt: 'O patrocínio não é para a sua conta mas para {account}. Assinatura recusada.', de: 'Die Übernahme gilt nicht Ihrem Konto, sondern {account}. Signatur abgelehnt.', fr: 'Le parrainage ne concerne pas votre compte mais {account}. Signature refusée.' },
  'guard.recoveryUnclosedSponsorship': { es: 'El patrocinio queda abierto y alcanzaría a lo próximo que cree tu cuenta. Firma rechazada.', en: 'The sponsorship is left open and would cover whatever your account creates next. Signature refused.', pt: 'O patrocínio fica aberto e abrangeria o que a sua conta criar a seguir. Assinatura recusada.', de: 'Die Übernahme bleibt offen und würde alles Weitere Ihres Kontos einschließen. Signatur abgelehnt.', fr: 'Le parrainage reste ouvert et couvrirait ce que votre compte créera ensuite. Signature refusée.' },
  'guard.recoveryExpectedSigner': { es: 'Se esperaba el alta de un firmante de recuperación y llegó otra operación ({op}). Firma rechazada.', en: 'A recovery signer was expected here and another operation arrived ({op}). Signature refused.', pt: 'Esperava-se o registo de um signatário de recuperação e chegou outra operação ({op}). Assinatura recusada.', de: 'Hier wurde ein Wiederherstellungs-Unterzeichner erwartet, es kam eine andere Operation ({op}). Signatur abgelehnt.', fr: 'Un signataire de récupération était attendu ici et une autre opération est arrivée ({op}). Signature refusée.' },
  'guard.recoveryExpectedThresholds': { es: 'Se esperaba el ajuste de umbrales y llegó otra operación ({op}). Firma rechazada.', en: 'The threshold change was expected here and another operation arrived ({op}). Signature refused.', pt: 'Esperava-se o ajuste de limiares e chegou outra operação ({op}). Assinatura recusada.', de: 'Hier wurde die Schwellenänderung erwartet, es kam eine andere Operation ({op}). Signatur abgelehnt.', fr: 'Le changement de seuils était attendu ici et une autre opération est arrivée ({op}). Signature refusée.' },
  'guard.recoveryUndecoded': { es: 'La wallet no pudo leer los datos de una operación de la transacción. Firma rechazada.', en: 'The wallet could not read one operation’s fields. Signature refused.', pt: 'A wallet não conseguiu ler os dados de uma operação da transação. Assinatura recusada.', de: 'Die Wallet konnte die Felder einer Operation nicht lesen. Signatur abgelehnt.', fr: 'Le portefeuille n’a pas pu lire les champs d’une opération. Signature refusée.' },
  'guard.recoverySignerKind': { es: 'El firmante que se agrega no es una clave normal ({kind}). Firma rechazada.', en: 'The signer being added is not an ordinary key ({kind}). Signature refused.', pt: 'O signatário que se adiciona não é uma chave normal ({kind}). Assinatura recusada.', de: 'Der hinzugefügte Unterzeichner ist kein gewöhnlicher Schlüssel ({kind}). Signatur abgelehnt.', fr: 'Le signataire ajouté n’est pas une clé ordinaire ({kind}). Signature refusée.' },
  'guard.recoveryUnknownSigner': { es: 'La transacción agrega un firmante que no es ninguno de los dos servidores ({key}). Firma rechazada.', en: 'This transaction adds a signer that is neither of the two servers ({key}). Signature refused.', pt: 'A transação adiciona um signatário que não é nenhum dos dois servidores ({key}). Assinatura recusada.', de: 'Diese Transaktion fügt einen Unterzeichner hinzu, der keiner der beiden Server ist ({key}). Signatur abgelehnt.', fr: 'Cette transaction ajoute un signataire qui n’est aucun des deux serveurs ({key}). Signature refusée.' },
  'guard.recoveryDuplicateSigner': { es: 'La misma clave se agrega dos veces ({key}). Un servidor tendría las dos partes. Firma rechazada.', en: 'The same key is added twice ({key}). One server would hold both shares. Signature refused.', pt: 'A mesma chave é adicionada duas vezes ({key}). Um servidor teria as duas partes. Assinatura recusada.', de: 'Derselbe Schlüssel wird zweimal hinzugefügt ({key}). Ein Server hätte beide Anteile. Signatur abgelehnt.', fr: 'La même clé est ajoutée deux fois ({key}). Un serveur détiendrait les deux parts. Signature refusée.' },
  'guard.recoverySignerWeight': { es: 'El firmante {key} viene con peso {weight} en lugar de {expected}: podría actuar solo. Firma rechazada.', en: 'Signer {key} carries weight {weight} instead of {expected}: it could act alone. Signature refused.', pt: 'O signatário {key} vem com peso {weight} em vez de {expected}: poderia agir sozinho. Assinatura recusada.', de: 'Unterzeichner {key} hat Gewicht {weight} statt {expected}: er könnte allein handeln. Signatur abgelehnt.', fr: 'Le signataire {key} porte le poids {weight} au lieu de {expected} : il pourrait agir seul. Signature refusée.' },
  'guard.recoveryThresholdSigner': { es: 'La operación de umbrales agrega además un firmante. Firma rechazada.', en: 'The threshold operation also adds a signer. Signature refused.', pt: 'A operação de limiares adiciona também um signatário. Assinatura recusada.', de: 'Die Schwellen-Operation fügt zusätzlich einen Unterzeichner hinzu. Signatur abgelehnt.', fr: 'L’opération de seuils ajoute aussi un signataire. Signature refusée.' },
  'guard.recoveryMasterWeight': { es: 'Tu clave quedaría con peso {weight} en lugar de {expected} y perderías el control de la cuenta. Firma rechazada.', en: 'Your own key would be left at weight {weight} instead of {expected}, losing control of the account. Signature refused.', pt: 'A sua chave ficaria com peso {weight} em vez de {expected} e perderia o controlo da conta. Assinatura recusada.', de: 'Ihr eigener Schlüssel bliebe bei Gewicht {weight} statt {expected} — Sie verlören die Kontrolle über das Konto. Signatur abgelehnt.', fr: 'Votre clé resterait au poids {weight} au lieu de {expected}, vous perdriez le contrôle du compte. Signature refusée.' },
  'guard.recoveryThreshold': { es: 'Un umbral queda en {weight} en lugar de {expected}. Firma rechazada.', en: 'A threshold is left at {weight} instead of {expected}. Signature refused.', pt: 'Um limiar fica em {weight} em vez de {expected}. Assinatura recusada.', de: 'Eine Schwelle bleibt bei {weight} statt {expected}. Signatur abgelehnt.', fr: 'Un seuil reste à {weight} au lieu de {expected}. Signature refusée.' },
  'guard.recoveryExtraOption': { es: 'La transacción cambia algo más de tu cuenta además de los firmantes. Firma rechazada.', en: 'This transaction changes something else about your account besides the signers. Signature refused.', pt: 'A transação altera algo mais da sua conta além dos signatários. Assinatura recusada.', de: 'Diese Transaktion ändert außer den Unterzeichnern noch etwas an Ihrem Konto. Signatur abgelehnt.', fr: 'Cette transaction modifie autre chose que les signataires de votre compte. Signature refusée.' },
  'guard.val.cleared': { es: '(se borra)', en: '(cleared)', pt: '(apagado)', de: '(wird gelöscht)', fr: '(effacé)' },
  'guard.recoveryNotSponsorSignature': { es: 'Quien dice pagar la reserva ({source}) no es quien firmó la transacción. Firma rechazada.', en: 'The account that says it pays the reserve ({source}) is not the one that signed. Signature refused.', pt: 'Quem diz pagar a reserva ({source}) não é quem assinou a transação. Assinatura recusada.', de: 'Das Konto, das die Reserve zahlen will ({source}), ist nicht das, welches signiert hat. Signatur abgelehnt.', fr: 'Le compte qui dit payer la réserve ({source}) n’est pas celui qui a signé. Signature refusée.' },
  ...
  /* Pollar — what its existing wallets still need: signing, signing out, and the reconnect
     the migration offers when a session expired (src/lib/pollarMigration.ts). */
  'net.pollarMainnetOnly': { es: 'Tu cuenta de Google vive en mainnet: no tiene dirección en otras redes. Para testnet cambiá a la wallet local que se creó con este mismo inicio de sesión.', en: 'Your Google account lives on mainnet: it has no address on other networks. For testnet, switch to the local wallet created by this same sign-in.', pt: 'A sua conta Google vive na mainnet: não tem endereço noutras redes. Para testnet, mude para a wallet local criada por este mesmo início de sessão.', de: 'Ihr Google-Konto lebt im Mainnet: In anderen Netzen hat es keine Adresse. Für Testnet wechseln Sie zur lokalen Wallet, die dieselbe Anmeldung erstellt hat.', fr: 'Votre compte Google vit sur le mainnet : il n’a d’adresse sur aucun autre réseau. Pour le testnet, basculez vers le portefeuille local créé par cette même connexion.' },
  ...
  'pollar.verifySent': { es: 'Te enviamos un código a tu correo.', en: 'We emailed you a code.', pt: 'Enviámos-lhe um código por e-mail.', de: 'Wir haben Ihnen einen Code per E-Mail gesendet.', fr: 'Nous vous avons envoyé un code par e-mail.' },
  'pollar.verifyInvalid': { es: 'Código incorrecto. Te quedan {n} intentos.', en: 'Incorrect code. {n} attempts left.', pt: 'Código incorreto. Restam {n} tentativas.', de: 'Falscher Code. Noch {n} Versuche.', fr: 'Code incorrect. Il reste {n} tentatives.' },
  'pollar.verifyExpired': { es: 'El código venció. Empezá un inicio de sesión nuevo.', en: 'The code expired. Start a new sign-in.', pt: 'O código expirou. Comece um novo início de sessão.', de: 'Der Code ist abgelaufen. Starten Sie eine neue Anmeldung.', fr: 'Le code a expiré. Recommencez la connexion.' },
  'pollar.verifyLocked': { es: 'Demasiados intentos. Empezá un inicio de sesión nuevo.', en: 'Too many attempts. Start a new sign-in.', pt: 'Demasiadas tentativas. Comece um novo início de sessão.', de: 'Zu viele Versuche. Starten Sie eine neue Anmeldung.', fr: 'Trop de tentatives. Recommencez la connexion.' },
  'pollar.identityMismatch': { es: 'Ese inicio de sesión es de otro correo, no del de tu cuenta de Cosmos Pay. Entrá con el mismo correo.', en: 'That sign-in belongs to a different email than your Cosmos Pay account. Sign in with the same email.', pt: 'Esse início de sessão é de outro e-mail, não do da sua conta Cosmos Pay. Entre com o mesmo e-mail.', de: 'Diese Anmeldung gehört zu einer anderen E-Mail als Ihr Cosmos-Pay-Konto. Melden Sie sich mit derselben E-Mail an.', fr: 'Cette connexion correspond à un autre e-mail que celui de votre compte Cosmos Pay. Connectez-vous avec le même e-mail.' },
  'pollar.identityRequired': { es: 'Tu cuenta de Cosmos Pay no tiene un correo verificado, así que no puede usarse para entrar con una cuenta social.', en: 'Your Cosmos Pay account has no verified email, so it cannot be used for a social sign-in.', pt: 'A sua conta Cosmos Pay não tem um e-mail verificado, por isso não pode ser usada para um início de sessão social.', de: 'Ihr Cosmos-Pay-Konto hat keine verifizierte E-Mail und kann daher nicht für eine soziale Anmeldung verwendet werden.', fr: 'Votre compte Cosmos Pay n’a pas d’e-mail vérifié : il ne peut pas servir à une connexion sociale.' },
  ...
  /* The wallet's own sign-in (src/lib/signIn.ts) and the cloud backup it restores. */
  'signin.cta': { es: 'Entrar con Google, GitHub o email', en: 'Sign in with Google, GitHub or email', pt: 'Entrar com Google, GitHub ou e-mail', de: 'Mit Google, GitHub oder E-Mail anmelden', fr: 'Se connecter avec Google, GitHub ou e-mail' },
  'signin.title': { es: 'Iniciar sesión', en: 'Sign in', pt: 'Iniciar sessão', de: 'Anmelden', fr: 'Se connecter' },
  'signin.desc': { es: 'Entrá para crear tu wallet, o para recuperar la que ya tenés en otro dispositivo.', en: 'Sign in to create your wallet, or to restore the one you already have on another device.', pt: 'Inicie sessão para criar a sua wallet ou recuperar a que já tem noutro dispositivo.', de: 'Melden Sie sich an, um Ihre Wallet zu erstellen oder die wiederherzustellen, die Sie bereits auf einem anderen Gerät haben.', fr: 'Connectez-vous pour créer votre portefeuille, ou pour restaurer celui que vous avez déjà sur un autre appareil.' },
  'signin.keyNote': { es: 'Tu clave se genera en este dispositivo y nunca sale de él sin cifrar. Guardamos una copia cifrada que solo abre tu contraseña: si la olvidás, nadie puede recuperarla por vos.', en: 'Your key is generated on this device and never leaves it unencrypted. We keep an encrypted copy that only your password opens — if you forget it, nobody can recover it for you.', pt: 'A sua chave é gerada neste dispositivo e nunca sai dele sem cifra. Guardamos uma cópia cifrada que só a sua palavra-passe abre: se a esquecer, ninguém a pode recuperar por si.', de: 'Ihr Schlüssel wird auf diesem Gerät erzeugt und verlässt es nie unverschlüsselt. Wir bewahren eine verschlüsselte Kopie auf, die nur Ihr Passwort öffnet – wenn Sie es vergessen, kann es niemand für Sie wiederherstellen.', fr: 'Votre clé est générée sur cet appareil et ne le quitte jamais en clair. Nous conservons une copie chiffrée que seul votre mot de passe ouvre : si vous l’oubliez, personne ne peut la récupérer pour vous.' },
  'signin.continueWith': { es: 'Continuar con {provider}', en: 'Continue with {provider}', pt: 'Continuar com {provider}', de: 'Weiter mit {provider}', fr: 'Continuer avec {provider}' },
  'signin.or': { es: 'o', en: 'or', pt: 'ou', de: 'oder', fr: 'ou' },
  'signin.emailPlaceholder': { es: 'nombre@email.com', en: 'name@email.com', pt: 'nome@email.com', de: 'name@email.com', fr: 'nom@email.com' },
  'signin.emailCta': { es: 'Continuar con email', en: 'Continue with email', pt: 'Continuar com e-mail', de: 'Weiter mit E-Mail', fr: 'Continuer avec l’e-mail' },
  'signin.opening': { es: 'Abriendo el inicio de sesión…', en: 'Opening the sign-in…', pt: 'A abrir o início de sessão…', de: 'Anmeldung wird geöffnet…', fr: 'Ouverture de la connexion…' },
  'signin.waiting': { es: 'Esperando a que termines en el navegador…', en: 'Waiting for you to finish in the browser…', pt: 'A aguardar que termine no navegador…', de: 'Warten, bis Sie im Browser fertig sind…', fr: 'En attente de la fin de la connexion dans le navigateur…' },
  'signin.claiming': { es: 'Verificando tu cuenta…', en: 'Checking your account…', pt: 'A verificar a sua conta…', de: 'Ihr Konto wird geprüft…', fr: 'Vérification de votre compte…' },
  'signin.copyLink': { es: 'Copiar enlace', en: 'Copy link', pt: 'Copiar ligação', de: 'Link kopieren', fr: 'Copier le lien' },
  'signin.openFailed': { es: 'No se pudo abrir el navegador. Copiá el enlace y abrilo a mano.', en: 'Could not open the browser. Copy the link and open it yourself.', pt: 'Não foi possível abrir o navegador. Copie a ligação e abra-a manualmente.', de: 'Der Browser konnte nicht geöffnet werden. Kopieren Sie den Link und öffnen Sie ihn selbst.', fr: 'Impossible d’ouvrir le navigateur. Copiez le lien et ouvrez-le vous-même.' },
  'signin.codeTitle': { es: 'Revisá tu correo', en: 'Check your email', pt: 'Verifique o seu e-mail', de: 'Prüfen Sie Ihre E-Mails', fr: 'Consultez vos e-mails' },
  'signin.codeDesc': { es: 'Enviamos un código de 6 dígitos a {email}. Ingresalo para entrar.', en: 'We sent a 6-digit code to {email}. Enter it to sign in.', pt: 'Enviámos um código de 6 dígitos para {email}. Introduza-o para entrar.', de: 'Wir haben einen 6-stelligen Code an {email} gesendet. Geben Sie ihn ein, um sich anzumelden.', fr: 'Nous avons envoyé un code à 6 chiffres à {email}. Saisissez-le pour vous connecter.' },
  'signin.codeDescExisting': { es: '{email} ya tiene una cuenta de Cosmos Pay, así que también te pedimos el código que le enviamos. Así nadie puede entrar a tu cuenta solo con un enlace de inicio de sesión.', en: '{email} already has a Cosmos Pay account, so we also need the code we just sent to it. That way nobody can get into your account with a sign-in link alone.', pt: '{email} já tem uma conta Cosmos Pay, por isso também precisamos do código que lhe enviámos. Assim ninguém entra na sua conta só com uma ligação de início de sessão.', de: '{email} hat bereits ein Cosmos-Pay-Konto, daher benötigen wir auch den Code, den wir gerade dorthin gesendet haben. So kann niemand allein mit einem Anmeldelink auf Ihr Konto zugreifen.', fr: '{email} a déjà un compte Cosmos Pay, nous avons donc aussi besoin du code que nous venons d’y envoyer. Ainsi, personne ne peut accéder à votre compte avec un simple lien de connexion.' },
  'signin.codeCta': { es: 'Confirmar', en: 'Confirm', pt: 'Confirmar', de: 'Bestätigen', fr: 'Confirmer' },
  'signin.codeSentExisting': { es: 'Te enviamos un código a tu correo.', en: 'We emailed you a code.', pt: 'Enviámos-lhe um código por e-mail.', de: 'Wir haben Ihnen einen Code per E-Mail gesendet.', fr: 'Nous vous avons envoyé un code par e-mail.' },
  'signin.codeInvalid': { es: 'Código incorrecto. Te quedan {n} intentos.', en: 'Incorrect code. {n} attempts left.', pt: 'Código incorreto. Restam {n} tentativas.', de: 'Falscher Code. Noch {n} Versuche.', fr: 'Code incorrect. Il vous reste {n} tentatives.' },
  'signin.codeExpired': { es: 'El código venció. Pedí uno nuevo.', en: 'The code expired. Ask for a new one.', pt: 'O código expirou. Peça um novo.', de: 'Der Code ist abgelaufen. Fordern Sie einen neuen an.', fr: 'Le code a expiré. Demandez-en un nouveau.' },
  'signin.codeLocked': { es: 'Demasiados intentos. Empezá de nuevo.', en: 'Too many attempts. Start again.', pt: 'Demasiadas tentativas. Comece de novo.', de: 'Zu viele Versuche. Beginnen Sie von vorn.', fr: 'Trop de tentatives. Recommencez.' },
  'signin.error.cancelled': { es: 'Inicio de sesión cancelado.', en: 'Sign-in cancelled.', pt: 'Início de sessão cancelado.', de: 'Anmeldung abgebrochen.', fr: 'Connexion annulée.' },
  'signin.error.timeout': { es: 'El inicio de sesión tardó demasiado. Probá de nuevo.', en: 'The sign-in took too long. Try again.', pt: 'O início de sessão demorou demasiado. Tente novamente.', de: 'Die Anmeldung hat zu lange gedauert. Versuchen Sie es erneut.', fr: 'La connexion a pris trop de temps. Réessayez.' },

  /* SEP-10 challenge refusals and SEP-30 recovery failures (`lib/sep10.ts`, `lib/recovery.ts`). */
  'sep10.error.undecodable': { es: 'El servidor de recuperación mandó algo que no es una transacción.', en: 'The recovery server sent something that is not a transaction.', pt: 'O servidor de recuperação enviou algo que não é uma transação.', de: 'Der Wiederherstellungsserver hat etwas gesendet, das keine Transaktion ist.', fr: 'Le serveur de récupération a envoyé quelque chose qui n’est pas une transaction.' },
  'sep10.error.feeBump': { es: 'El servidor mandó un envoltorio fee-bump en lugar de un desafío.', en: 'The server sent a fee-bump wrapper instead of a challenge.', pt: 'O servidor enviou um invólucro fee-bump em vez de um desafio.', de: 'Der Server hat eine Fee-Bump-Hülle statt einer Challenge gesendet.', fr: 'Le serveur a envoyé une enveloppe fee-bump au lieu d’un défi.' },
  'sep10.error.sequence': { es: 'Lo que mandó el servidor sí podría ejecutarse en la red. No es un desafío y no se firma.', en: 'What the server sent could actually run on the network. That is not a challenge, and it will not be signed.', pt: 'O que o servidor enviou poderia mesmo ser executado na rede. Não é um desafio e não será assinado.', de: 'Was der Server gesendet hat, könnte tatsächlich im Netzwerk ausgeführt werden. Das ist keine Challenge und wird nicht signiert.', fr: 'Ce que le serveur a envoyé pourrait réellement s’exécuter sur le réseau. Ce n’est pas un défi et il ne sera pas signé.' },
  'sep10.error.signingKey': { es: 'El desafío no viene firmado por la clave que el servidor publica.', en: 'The challenge is not signed by the key the server publishes.', pt: 'O desafio não vem assinado pela chave que o servidor publica.', de: 'Die Challenge ist nicht mit dem Schlüssel signiert, den der Server veröffentlicht.', fr: 'Le défi n’est pas signé par la clé que le serveur publie.' },
  'sep10.error.ourSource': { es: 'El desafío viene emitido desde tu propia cuenta.', en: 'The challenge is sourced from your own account.', pt: 'O desafio vem emitido a partir da sua própria conta.', de: 'Die Challenge stammt von Ihrem eigenen Konto.', fr: 'Le défi provient de votre propre compte.' },
  'sep10.error.memo': { es: 'El desafío trae un memo, y un desafío no lleva ninguno.', en: 'The challenge carries a memo, and a challenge carries none.', pt: 'O desafio traz um memo, e um desafio não leva nenhum.', de: 'Die Challenge enthält ein Memo — eine Challenge enthält keines.', fr: 'Le défi porte un mémo, alors qu’un défi n’en porte aucun.' },
  'sep10.error.operations': { es: 'El desafío contiene operaciones que no corresponden.', en: 'The challenge contains operations that do not belong in one.', pt: 'O desafio contém operações que não correspondem.', de: 'Die Challenge enthält Operationen, die nicht hineingehören.', fr: 'Le défi contient des opérations qui n’y ont pas leur place.' },
  'sep10.error.notOurs': { es: 'El desafío no está dirigido a tu cuenta.', en: 'The challenge is not addressed to your account.', pt: 'O desafio não está dirigido à sua conta.', de: 'Die Challenge ist nicht an Ihr Konto gerichtet.', fr: 'Le défi ne s’adresse pas à votre compte.' },
  'sep10.error.homeDomain': { es: 'El desafío nombra otro dominio.', en: 'The challenge names a different domain.', pt: 'O desafio nomeia outro domínio.', de: 'Die Challenge nennt eine andere Domain.', fr: 'Le défi nomme un autre domaine.' },
  'sep10.error.nonce': { es: 'El desafío no trae datos nuevos: firmarlo valdría para siempre.', en: 'The challenge carries no fresh randomness: signing it would be good forever.', pt: 'O desafio não traz dados novos: assiná-lo valeria para sempre.', de: 'Die Challenge enthält keine frische Zufälligkeit: sie zu signieren gälte für immer.', fr: 'Le défi ne contient aucune donnée aléatoire fraîche : le signer vaudrait pour toujours.' },
  'sep10.error.extraOnUs': { es: 'El desafío escribiría datos en tu cuenta.', en: 'The challenge would write data on your account.', pt: 'O desafio escreveria dados na sua conta.', de: 'Die Challenge würde Daten in Ihr Konto schreiben.', fr: 'Le défi écrirait des données sur votre compte.' },
  'sep10.error.noWebAuthDomain': { es: 'El desafío no dice para qué servidor es.', en: 'The challenge does not say which server it is for.', pt: 'O desafio não diz para que servidor é.', de: 'Die Challenge sagt nicht, für welchen Server sie gilt.', fr: 'Le défi n’indique pas à quel serveur il est destiné.' },
  'sep10.error.webAuthDomain': { es: 'El desafío es para otro servidor. No se firma acá.', en: 'The challenge is for another server. It will not be signed here.', pt: 'O desafio é para outro servidor. Não será assinado aqui.', de: 'Die Challenge gilt einem anderen Server. Sie wird hier nicht signiert.', fr: 'Le défi est destiné à un autre serveur. Il ne sera pas signé ici.' },
  'sep10.error.noWindow': { es: 'El desafío no caduca nunca.', en: 'The challenge never expires.', pt: 'O desafio nunca caduca.', de: 'Die Challenge läuft nie ab.', fr: 'Le défi n’expire jamais.' },
  'sep10.error.expired': { es: 'El desafío ya venció. Probá de nuevo.', en: 'The challenge has expired. Try again.', pt: 'O desafio já expirou. Tente novamente.', de: 'Die Challenge ist abgelaufen. Versuchen Sie es erneut.', fr: 'Le défi a expiré. Réessayez.' },
  'sep10.error.notYetValid': { es: 'El desafío todavía no es válido. Revisá la hora del dispositivo.', en: 'The challenge is not valid yet. Check the device clock.', pt: 'O desafio ainda não é válido. Verifique a hora do dispositivo.', de: 'Die Challenge ist noch nicht gültig. Prüfen Sie die Uhrzeit des Geräts.', fr: 'Le défi n’est pas encore valide. Vérifiez l’heure de l’appareil.' },
  'sep10.error.window': { es: 'El desafío vale demasiado tiempo para ser uno.', en: 'The challenge stays valid far too long to be one.', pt: 'O desafio é válido demasiado tempo para ser um.', de: 'Die Challenge bleibt viel zu lange gültig, um eine zu sein.', fr: 'Le défi reste valide bien trop longtemps pour en être un.' },
  'sep10.error.wrongKey': { es: 'La clave de este dispositivo no es la de esa cuenta.', en: 'This device’s key is not the one for that account.', pt: 'A chave deste dispositivo não é a dessa conta.', de: 'Der Schlüssel dieses Geräts gehört nicht zu diesem Konto.', fr: 'La clé de cet appareil n’est pas celle de ce compte.' },
  'recovery.error.notConfigured': { es: 'Esta versión de la wallet no tiene servidores de recuperación configurados.', en: 'This build of the wallet has no recovery servers configured.', pt: 'Esta versão da wallet não tem servidores de recuperação configurados.', de: 'Dieser Wallet-Build hat keine Wiederherstellungsserver konfiguriert.', fr: 'Cette version du portefeuille n’a aucun serveur de récupération configuré.' },
  'recovery.error.network': { es: '{server} trabaja en otra red que la tuya. Un firmante sirve en una sola.', en: '{server} is on a different network than yours. A signer only exists on one.', pt: '{server} trabalha noutra rede que não a sua. Um signatário só existe numa.', de: '{server} arbeitet in einem anderen Netzwerk als Ihrem. Ein Unterzeichner existiert nur in einem.', fr: '{server} est sur un réseau différent du vôtre. Un signataire n’existe que sur un seul.' },
  'recovery.error.domain': { es: '{server} dice llamarse de otra forma que la dirección a la que respondió.', en: '{server} claims a different name than the address that answered.', pt: '{server} diz chamar-se de outra forma que o endereço que respondeu.', de: '{server} nennt sich anders als die Adresse, die geantwortet hat.', fr: '{server} déclare un nom différent de l’adresse qui a répondu.' },
  'recovery.error.sameServer': { es: 'Los dos servidores de recuperación son el mismo. La recuperación necesita dos distintos.', en: 'Both recovery servers are the same one. Recovery needs two separate servers.', pt: 'Os dois servidores de recuperação são o mesmo. A recuperação precisa de dois distintos.', de: 'Beide Wiederherstellungsserver sind derselbe. Wiederherstellung braucht zwei getrennte.', fr: 'Les deux serveurs de récupération sont le même. La récupération en exige deux distincts.' },
  'recovery.error.homeDomain': { es: 'Los dos servidores de recuperación no se ponen de acuerdo en el dominio de la wallet.', en: 'The two recovery servers do not agree on the wallet’s domain.', pt: 'Os dois servidores de recuperação não concordam quanto ao domínio da wallet.', de: 'Die beiden Wiederherstellungsserver sind sich über die Domain der Wallet nicht einig.', fr: 'Les deux serveurs de récupération ne s’accordent pas sur le domaine du portefeuille.' },
  'recovery.error.noSigner': { es: '{server} no devolvió ningún firmante para tu cuenta.', en: '{server} returned no signer for your account.', pt: '{server} não devolveu nenhum signatário para a sua conta.', de: '{server} hat keinen Unterzeichner für Ihr Konto zurückgegeben.', fr: '{server} n’a renvoyé aucun signataire pour votre compte.' },
  'recovery.error.sameSigner': { es: 'Los dos servidores devolvieron la misma clave. No hay dos partes.', en: 'Both servers returned the same key. There are not two shares.', pt: 'Os dois servidores devolveram a mesma chave. Não há duas partes.', de: 'Beide Server haben denselben Schlüssel zurückgegeben. Es gibt keine zwei Anteile.', fr: 'Les deux serveurs ont renvoyé la même clé. Il n’y a pas deux parts.' },
  'recovery.error.signerIsAccount': { es: 'Un servidor devolvió tu propia cuenta como firmante.', en: 'A server returned your own account as its signer.', pt: 'Um servidor devolveu a sua própria conta como signatário.', de: 'Ein Server hat Ihr eigenes Konto als Unterzeichner zurückgegeben.', fr: 'Un serveur a renvoyé votre propre compte comme signataire.' },
  'recovery.error.badSignature': { es: 'La firma de {server} no corresponde a esta transacción.', en: 'The signature from {server} does not hold for this transaction.', pt: 'A assinatura de {server} não corresponde a esta transação.', de: 'Die Signatur von {server} passt nicht zu dieser Transaktion.', fr: 'La signature de {server} ne correspond pas à cette transaction.' },
  'recovery.error.noAccounts': { es: 'No encontramos ninguna cuenta que puedas recuperar con ese correo.', en: 'No account this email can recover was found.', pt: 'Não encontrámos nenhuma conta que possa recuperar com esse email.', de: 'Es wurde kein Konto gefunden, das diese E-Mail wiederherstellen kann.', fr: 'Aucun compte récupérable avec cet e-mail n’a été trouvé.' },
  'recovery.error.notFunded': { es: 'La cuenta todavía no existe en la red, así que no hay nada que proteger.', en: 'The account does not exist on the network yet, so there is nothing to protect.', pt: 'A conta ainda não existe na rede, por isso não há nada a proteger.', de: 'Das Konto existiert im Netzwerk noch nicht, es gibt also nichts zu schützen.', fr: 'Le compte n’existe pas encore sur le réseau, il n’y a donc rien à protéger.' },

  /* The recovery section in Settings (`features/settings/RecoverySection.tsx`). */
  'recovery.title': { es: 'Recuperación de cuenta', en: 'Account recovery', pt: 'Recuperação de conta', de: 'Kontowiederherstellung', fr: 'Récupération du compte' },
  'recovery.what': { es: 'Si perdés el dispositivo, dos servidores pueden devolverte la cuenta entre los dos. Ninguno puede solo.', en: 'If you lose the device, two servers can give the account back — together. Neither can alone.', pt: 'Se perder o dispositivo, dois servidores podem devolver-lhe a conta — juntos. Nenhum consegue sozinho.', de: 'Wenn Sie das Gerät verlieren, können zwei Server das Konto zurückgeben — gemeinsam. Keiner allein.', fr: 'Si vous perdez l’appareil, deux serveurs peuvent vous rendre le compte — ensemble. Aucun ne le peut seul.' },
  'recovery.statusOn': { es: 'Activada', en: 'On', pt: 'Ativada', de: 'Aktiv', fr: 'Activée' },
  'recovery.statusOff': { es: 'Desactivada', en: 'Off', pt: 'Desativada', de: 'Inaktiv', fr: 'Désactivée' },
  'recovery.turnOn': { es: 'Activar recuperación', en: 'Turn recovery on', pt: 'Ativar recuperação', de: 'Wiederherstellung aktivieren', fr: 'Activer la récupération' },
  'recovery.turnOff': { es: 'Desactivar', en: 'Turn off', pt: 'Desativar', de: 'Deaktivieren', fr: 'Désactiver' },
  'recovery.bargain': { es: 'Quien pueda demostrar que {email} es suyo —ante los dos servidores— va a poder poner una clave nueva en esta cuenta. Ese es el trato: es lo que te devuelve la cuenta si perdés el teléfono.', en: 'Whoever can prove {email} is theirs — to both servers — will be able to put a new key on this account. That is the bargain: it is what gives the account back if you lose your phone.', pt: 'Quem conseguir provar que {email} é seu — perante os dois servidores — poderá colocar uma chave nova nesta conta. É esse o acordo: é o que lhe devolve a conta se perder o telemóvel.', de: 'Wer nachweisen kann, dass {email} ihm gehört — gegenüber beiden Servern —, kann einen neuen Schlüssel auf dieses Konto setzen. Das ist der Handel: genau das gibt Ihnen das Konto zurück, wenn Sie Ihr Telefon verlieren.', fr: 'Quiconque peut prouver que {email} lui appartient — auprès des deux serveurs — pourra placer une nouvelle clé sur ce compte. C’est le marché : c’est ce qui vous rend le compte si vous perdez votre téléphone.' },
  'recovery.reserveNote': { es: 'Cada firmante ocupa 0,5 XLM de reserva en tu cuenta, y los recuperás al desactivarla.', en: 'Each signer holds 0.5 XLM of reserve on your account, and you get it back when you turn recovery off.', pt: 'Cada signatário ocupa 0,5 XLM de reserva na sua conta, e recupera-o ao desativar.', de: 'Jeder Unterzeichner bindet 0,5 XLM Reserve auf Ihrem Konto; beim Deaktivieren erhalten Sie sie zurück.', fr: 'Chaque signataire immobilise 0,5 XLM de réserve sur votre compte, restituée à la désactivation.' },
  'recovery.sponsoredNote': { es: 'Esta cuenta no tiene XLM de sobra para pagar la reserva de los dos firmantes, así que la paga Cosmos Pay. Te mandamos un código al correo para confirmar que sos vos.', en: 'This account has no spare XLM to pay the two signers’ reserve, so Cosmos Pay pays it. We will email you a code to confirm it is you.', pt: 'Esta conta não tem XLM de sobra para pagar a reserva dos dois signatários, por isso é a Cosmos Pay que a paga. Enviamos-lhe um código por e-mail para confirmar que é você.', de: 'Dieses Konto hat kein übriges XLM für die Reserve der beiden Unterzeichner, daher zahlt Cosmos Pay sie. Wir senden Ihnen einen Code per E-Mail, um zu bestätigen, dass Sie es sind.', fr: 'Ce compte n’a pas de XLM disponible pour payer la réserve des deux signataires, c’est donc Cosmos Pay qui la paie. Nous vous enverrons un code par e-mail pour confirmer que c’est bien vous.' },
  'recovery.sponsoredCta': { es: 'Enviarme el código', en: 'Email me the code', pt: 'Enviar-me o código', de: 'Code zusenden', fr: 'M’envoyer le code' },
  'recovery.confirmTitle': { es: 'Activar recuperación', en: 'Turn recovery on', pt: 'Ativar recuperação', de: 'Wiederherstellung aktivieren', fr: 'Activer la récupération' },
  'recovery.confirmMsg': { es: 'Se van a agregar dos firmantes a tu cuenta, y {email} va a poder recuperarla. Tu clave sigue alcanzando sola para firmar.', en: 'Two signers will be added to your account, and {email} will be able to recover it. Your own key still signs on its own.', pt: 'Vão ser adicionados dois signatários à sua conta, e {email} poderá recuperá-la. A sua chave continua a assinar sozinha.', de: 'Ihrem Konto werden zwei Unterzeichner hinzugefügt, und {email} kann es wiederherstellen. Ihr eigener Schlüssel signiert weiterhin allein.', fr: 'Deux signataires seront ajoutés à votre compte, et {email} pourra le récupérer. Votre propre clé signe toujours seule.' },
  'recovery.enabled': { es: 'Recuperación activada.', en: 'Recovery is on.', pt: 'Recuperação ativada.', de: 'Wiederherstellung ist aktiv.', fr: 'Récupération activée.' },
  'recovery.disabled': { es: 'Recuperación desactivada.', en: 'Recovery is off.', pt: 'Recuperação desativada.', de: 'Wiederherstellung ist deaktiviert.', fr: 'Récupération désactivée.' },
  'recovery.offConfirmTitle': { es: 'Desactivar recuperación', en: 'Turn recovery off', pt: 'Desativar recuperação', de: 'Wiederherstellung deaktivieren', fr: 'Désactiver la récupération' },
  'recovery.offConfirmMsg': { es: 'Se quitan los dos firmantes. Si perdés el dispositivo, nadie va a poder devolverte la cuenta.', en: 'Both signers are removed. If you lose the device, nobody will be able to give the account back.', pt: 'Ambos os signatários são removidos. Se perder o dispositivo, ninguém poderá devolver-lhe a conta.', de: 'Beide Unterzeichner werden entfernt. Wenn Sie das Gerät verlieren, kann Ihnen niemand das Konto zurückgeben.', fr: 'Les deux signataires sont retirés. Si vous perdez l’appareil, personne ne pourra vous rendre le compte.' },
  'recovery.emailRegistered': { es: 'Recupera esta cuenta', en: 'Recovers this account', pt: 'Recupera esta conta', de: 'Stellt dieses Konto wieder her', fr: 'Récupère ce compte' },
  'recovery.emailUnknown': { es: 'correo desconocido', en: 'email unknown', pt: 'email desconhecido', de: 'E-Mail unbekannt', fr: 'e-mail inconnu' },
  'recovery.emailUnknownNote': { es: 'La recuperación se activó en otro dispositivo, así que acá no sabemos con qué correo quedó registrada. Podés volver a registrarla con {email}, el correo de esta wallet.', en: 'Recovery was turned on from another device, so this one does not know which email it was registered with. You can register it again with {email}, this wallet’s email.', pt: 'A recuperação foi ativada noutro dispositivo, por isso aqui não sabemos com que email ficou registada. Pode registá-la de novo com {email}, o email desta wallet.', de: 'Die Wiederherstellung wurde auf einem anderen Gerät aktiviert; dieses weiß daher nicht, mit welcher E-Mail sie registriert wurde. Sie können sie erneut mit {email} registrieren, der E-Mail dieser Wallet.', fr: 'La récupération a été activée depuis un autre appareil ; celui-ci ignore donc avec quel e-mail elle a été enregistrée. Vous pouvez l’enregistrer à nouveau avec {email}, l’e-mail de ce portefeuille.' },
  'recovery.emailDrifted': { es: 'Cambiaste el correo de la wallet, pero los servidores siguen teniendo {registered}. Hoy quien recupera esta cuenta es ese correo, no {email}.', en: 'You changed this wallet’s email, but the servers still hold {registered}. Today it is that address that recovers this account, not {email}.', pt: 'Mudou o email da wallet, mas os servidores continuam com {registered}. Hoje quem recupera esta conta é esse email, não {email}.', de: 'Sie haben die E-Mail dieser Wallet geändert, die Server haben aber weiterhin {registered}. Heute stellt diese Adresse das Konto wieder her, nicht {email}.', fr: 'Vous avez changé l’e-mail de ce portefeuille, mais les serveurs conservent {registered}. Aujourd’hui, c’est cette adresse qui récupère ce compte, pas {email}.' },
  'recovery.emailUpdate': { es: 'Actualizar el correo de recuperación', en: 'Update the recovery email', pt: 'Atualizar o email de recuperação', de: 'Wiederherstellungs-E-Mail aktualisieren', fr: 'Mettre à jour l’e-mail de récupération' },
  'recovery.emailConfirmTitle': { es: 'Cambiar quién puede recuperar', en: 'Change who can recover', pt: 'Alterar quem pode recuperar', de: 'Ändern, wer wiederherstellen darf', fr: 'Changer qui peut récupérer' },
  'recovery.emailConfirmMsg': { es: 'A partir de ahora la cuenta la va a recuperar {email}, y el correo anterior deja de servir. No cambia nada más: los firmantes y tu clave siguen igual.', en: 'From now on {email} is what recovers this account, and the previous address stops working. Nothing else changes: the signers and your own key stay as they are.', pt: 'A partir de agora é {email} que recupera esta conta, e o email anterior deixa de servir. Nada mais muda: os signatários e a sua chave ficam iguais.', de: 'Ab jetzt stellt {email} dieses Konto wieder her, und die bisherige Adresse funktioniert nicht mehr. Sonst ändert sich nichts: Die Unterzeichner und Ihr eigener Schlüssel bleiben unverändert.', fr: 'Désormais, c’est {email} qui récupère ce compte, et l’adresse précédente cesse de fonctionner. Rien d’autre ne change : les signataires et votre propre clé restent identiques.' },
  'recovery.emailUpdated': { es: 'Correo de recuperación actualizado.', en: 'The recovery email is updated.', pt: 'Email de recuperação atualizado.', de: 'Die Wiederherstellungs-E-Mail wurde aktualisiert.', fr: 'L’e-mail de récupération est à jour.' },
  'recovery.offWarning': { es: 'Sin recuperación, perder el dispositivo y la contraseña es perder la cuenta.', en: 'Without recovery, losing the device and the password is losing the account.', pt: 'Sem recuperação, perder o dispositivo e a palavra-passe é perder a conta.', de: 'Ohne Wiederherstellung bedeutet der Verlust von Gerät und Passwort den Verlust des Kontos.', fr: 'Sans récupération, perdre l’appareil et le mot de passe, c’est perdre le compte.' },
  'recovery.error.noEmail': { es: 'Esta wallet no tiene un correo con el que recuperarla. Agregalo en tu perfil.', en: 'This wallet has no email to recover it with. Add one in your profile.', pt: 'Esta wallet não tem um email com que a recuperar. Adicione um no seu perfil.', de: 'Diese Wallet hat keine E-Mail zur Wiederherstellung. Fügen Sie eine im Profil hinzu.', fr: 'Ce portefeuille n’a pas d’e-mail pour le récupérer. Ajoutez-en un dans votre profil.' },
  'recovery.error.pollar': { es: 'Las cuentas de Pollar no usan una clave de este dispositivo, así que no hay nada que reemplazar.', en: 'Pollar accounts do not use a key from this device, so there is nothing to replace.', pt: 'As contas Pollar não usam uma chave deste dispositivo, portanto não há nada a substituir.', de: 'Pollar-Konten verwenden keinen Schlüssel dieses Geräts, es gibt also nichts zu ersetzen.', fr: 'Les comptes Pollar n’utilisent pas de clé de cet appareil, il n’y a donc rien à remplacer.' },
  'recovery.error.notOn': { es: 'La recuperación no está activada en esta cuenta.', en: 'Recovery is not on for this account.', pt: 'A recuperação não está ativada nesta conta.', de: 'Für dieses Konto ist die Wiederherstellung nicht aktiv.', fr: 'La récupération n’est pas activée pour ce compte.' },
  'recovery.error.generic': { es: 'No se pudo completar la operación de recuperación.', en: 'The recovery operation could not be completed.', pt: 'Não foi possível completar a operação de recuperação.', de: 'Der Wiederherstellungsvorgang konnte nicht abgeschlossen werden.', fr: 'L’opération de récupération n’a pas pu être effectuée.' },
  'recovery.error.badCode': { es: 'El código son seis dígitos.', en: 'The code is six digits.', pt: 'O código tem seis dígitos.', de: 'Der Code besteht aus sechs Ziffern.', fr: 'Le code comporte six chiffres.' },

  /* Recovering an account onto this device (`features/onboarding/RecoverAccount.tsx`). */
  'common.or': { es: 'o', en: 'or', pt: 'ou', de: 'oder', fr: 'ou' },
  'signin.recoverOffer': { es: 'Si activaste la recuperación de cuenta, dos servidores pueden devolvértela sin la contraseña. Conservás la dirección y todo lo que tiene.', en: 'If you turned account recovery on, two servers can give the account back without the password. You keep the address and everything on it.', pt: 'Se ativou a recuperação de conta, dois servidores podem devolvê-la sem a palavra-passe. Mantém o endereço e tudo o que tem.', de: 'Wenn Sie die Kontowiederherstellung aktiviert haben, können zwei Server das Konto ohne Passwort zurückgeben. Adresse und Inhalt bleiben erhalten.', fr: 'Si vous avez activé la récupération du compte, deux serveurs peuvent vous le rendre sans le mot de passe. Vous gardez l’adresse et tout ce qu’elle contient.' },
  'signin.recoverCta': { es: 'Recuperar mi cuenta', en: 'Recover my account', pt: 'Recuperar a minha conta', de: 'Mein Konto wiederherstellen', fr: 'Récupérer mon compte' },
  'recover.title': { es: 'Recuperar la cuenta', en: 'Recover the account', pt: 'Recuperar a conta', de: 'Konto wiederherstellen', fr: 'Récupérer le compte' },
  'recover.desc': { es: 'Los dos servidores de recuperación van a poner una clave nueva —la de este dispositivo— en tu cuenta. La dirección, el saldo y el historial quedan igual.', en: 'The two recovery servers will put a new key — this device’s — on your account. The address, the balance and the history stay as they are.', pt: 'Os dois servidores de recuperação vão colocar uma chave nova — a deste dispositivo — na sua conta. O endereço, o saldo e o histórico ficam iguais.', de: 'Die beiden Wiederherstellungsserver setzen einen neuen Schlüssel — den dieses Geräts — auf Ihr Konto. Adresse, Guthaben und Verlauf bleiben unverändert.', fr: 'Les deux serveurs de récupération placeront une nouvelle clé — celle de cet appareil — sur votre compte. L’adresse, le solde et l’historique restent identiques.' },
  'recover.looking': { es: 'Preguntando a los dos servidores…', en: 'Asking both servers…', pt: 'A perguntar aos dois servidores…', de: 'Beide Server werden gefragt …', fr: 'Interrogation des deux serveurs…' },
  'recover.newPwdLabel': { es: 'Contraseña nueva', en: 'New password', pt: 'Palavra-passe nova', de: 'Neues Passwort', fr: 'Nouveau mot de passe' },
  'recover.phraseWarn': { es: 'Tu frase de recuperación cambia. La nueva restaura una clave, no esta cuenta: para volver a esta cuenta desde otro dispositivo, usá la recuperación otra vez. Y la frase vieja, si aparece, ya no firma acá.', en: 'Your recovery phrase changes. The new one restores a KEY, not this account: to get back to this account from another device, use recovery again. And the old phrase, if it turns up, no longer signs here.', pt: 'A sua frase de recuperação muda. A nova restaura uma CHAVE, não esta conta: para voltar a esta conta noutro dispositivo, use a recuperação outra vez. E a frase antiga, se aparecer, já não assina aqui.', de: 'Ihre Wiederherstellungsphrase ändert sich. Die neue stellt einen SCHLÜSSEL wieder her, nicht dieses Konto: Um von einem anderen Gerät zurück zu diesem Konto zu gelangen, nutzen Sie erneut die Wiederherstellung. Und die alte Phrase signiert hier nicht mehr, falls sie wieder auftaucht.', fr: 'Votre phrase de récupération change. La nouvelle restaure une CLÉ, pas ce compte : pour revenir à ce compte depuis un autre appareil, utilisez à nouveau la récupération. Et l’ancienne phrase, si elle réapparaît, ne signe plus ici.' },
  'recover.phraseAck': { es: 'Entiendo que mi frase de recuperación cambia.', en: 'I understand my recovery phrase changes.', pt: 'Compreendo que a minha frase de recuperação muda.', de: 'Ich verstehe, dass sich meine Wiederherstellungsphrase ändert.', fr: 'Je comprends que ma phrase de récupération change.' },
  'recover.cta': { es: 'Recuperar la cuenta', en: 'Recover the account', pt: 'Recuperar a conta', de: 'Konto wiederherstellen', fr: 'Récupérer le compte' },
  'recovery.recovered': { es: 'Cuenta recuperada en este dispositivo.', en: 'The account is back on this device.', pt: 'Conta recuperada neste dispositivo.', de: 'Das Konto ist wieder auf diesem Gerät.', fr: 'Le compte est de retour sur cet appareil.' },
  'signin.error.expired': { es: 'El inicio de sesión venció. Empezá uno nuevo.', en: 'The sign-in expired. Start a new one.', pt: 'O início de sessão expirou. Comece um novo.', de: 'Die Anmeldung ist abgelaufen. Starten Sie eine neue.', fr: 'La connexion a expiré. Recommencez.' },
  'signin.error.failed': { es: 'No se pudo iniciar sesión. Probá de nuevo.', en: 'The sign-in failed. Try again.', pt: 'Não foi possível iniciar sessão. Tente novamente.', de: 'Die Anmeldung ist fehlgeschlagen. Versuchen Sie es erneut.', fr: 'La connexion a échoué. Réessayez.' },
  'signin.error.denied': { es: 'Se rechazó el permiso en el navegador. No se compartió nada.', en: 'Permission was declined in the browser. Nothing was shared.', pt: 'A permissão foi recusada no navegador. Nada foi partilhado.', de: 'Die Berechtigung wurde im Browser abgelehnt. Es wurde nichts geteilt.', fr: 'L’autorisation a été refusée dans le navigateur. Rien n’a été partagé.' },
  'signin.error.emailUnverified': { es: 'Esa cuenta no tiene un email verificado. Verificalo con el proveedor, o entrá con un código por email.', en: 'That account has no verified email. Verify it with the provider, or sign in with an email code.', pt: 'Essa conta não tem um e-mail verificado. Verifique-o com o fornecedor ou entre com um código por e-mail.', de: 'Dieses Konto hat keine bestätigte E-Mail-Adresse. Bestätigen Sie sie beim Anbieter oder melden Sie sich mit einem E-Mail-Code an.', fr: 'Ce compte n’a pas d’e-mail vérifié. Vérifiez-le auprès du fournisseur ou connectez-vous avec un code par e-mail.' },
  'signin.error.badUrl': { es: 'El servidor devolvió una dirección de inicio de sesión que no es segura.', en: 'The server returned a sign-in address that is not secure.', pt: 'O servidor devolveu um endereço de início de sessão que não é seguro.', de: 'Der Server hat eine unsichere Anmeldeadresse zurückgegeben.', fr: 'Le serveur a renvoyé une adresse de connexion non sécurisée.' },
  'signin.error.unavailable': { es: 'El inicio de sesión no está disponible ahora. Probá más tarde, o creá una wallet con frase de recuperación.', en: 'Sign-in is not available right now. Try later, or create a wallet with a recovery phrase.', pt: 'O início de sessão não está disponível agora. Tente mais tarde ou crie uma wallet com frase de recuperação.', de: 'Die Anmeldung ist gerade nicht verfügbar. Versuchen Sie es später oder erstellen Sie eine Wallet mit Wiederherstellungsphrase.', fr: 'La connexion n’est pas disponible pour le moment. Réessayez plus tard ou créez un portefeuille avec une phrase de récupération.' },
  'signin.passwordDesc': { es: 'Esta contraseña protege tu wallet en este dispositivo y también la copia cifrada que guardamos. La vas a necesitar para recuperar tu wallet en otro dispositivo, y nadie puede restablecerla por vos.', en: 'This password protects your wallet on this device and the encrypted copy we keep. You will need it to restore your wallet on another device, and nobody can reset it for you.', pt: 'Esta palavra-passe protege a sua wallet neste dispositivo e a cópia cifrada que guardamos. Vai precisar dela para recuperar a wallet noutro dispositivo, e ninguém a pode repor por si.', de: 'Dieses Passwort schützt Ihre Wallet auf diesem Gerät und die verschlüsselte Kopie, die wir aufbewahren. Sie benötigen es, um Ihre Wallet auf einem anderen Gerät wiederherzustellen, und niemand kann es für Sie zurücksetzen.', fr: 'Ce mot de passe protège votre portefeuille sur cet appareil et la copie chiffrée que nous conservons. Vous en aurez besoin pour restaurer votre portefeuille sur un autre appareil, et personne ne peut le réinitialiser pour vous.' },
  'signin.restoreTitle': { es: 'Recuperar tu wallet', en: 'Restore your wallet', pt: 'Recuperar a sua wallet', de: 'Wallet wiederherstellen', fr: 'Restaurer votre portefeuille' },
  'signin.restoreDesc': { es: '{email} tiene una wallet guardada ({address}). Ingresá la contraseña con la que la protegiste.', en: '{email} has a backed-up wallet ({address}). Enter the password you protected it with.', pt: '{email} tem uma wallet guardada ({address}). Introduza a palavra-passe com que a protegeu.', de: '{email} hat eine gesicherte Wallet ({address}). Geben Sie das Passwort ein, mit dem Sie sie geschützt haben.', fr: '{email} a un portefeuille sauvegardé ({address}). Saisissez le mot de passe avec lequel vous l’avez protégé.' },
  'signin.backupPwdLabel': { es: 'Contraseña de tu wallet', en: 'Your wallet password', pt: 'Palavra-passe da wallet', de: 'Passwort Ihrer Wallet', fr: 'Mot de passe du portefeuille' },
  'signin.restoreCta': { es: 'Recuperar', en: 'Restore', pt: 'Recuperar', de: 'Wiederherstellen', fr: 'Restaurer' },
  'signin.forgot': { es: '¿Olvidaste la contraseña?', en: 'Forgot the password?', pt: 'Esqueceu a palavra-passe?', de: 'Passwort vergessen?', fr: 'Mot de passe oublié ?' },
  'signin.startOverWarn': { es: 'Nadie puede abrir la copia sin esa contraseña, tampoco nosotros. Podés crear una wallet nueva que reemplace la copia guardada, pero la wallet {address} y sus fondos solo se recuperan con su frase de recuperación.', en: 'Nobody can open the backup without that password, including us. You can create a new wallet that replaces the stored backup, but wallet {address} and its funds can then only be recovered with its recovery phrase.', pt: 'Ninguém pode abrir a cópia sem essa palavra-passe, nem nós. Pode criar uma wallet nova que substitui a cópia guardada, mas a wallet {address} e os seus fundos só se recuperam com a frase de recuperação.', de: 'Niemand kann die Sicherung ohne dieses Passwort öffnen, auch wir nicht. Sie können eine neue Wallet erstellen, die die gespeicherte Sicherung ersetzt, aber die Wallet {address} und ihr Guthaben lassen sich dann nur noch mit ihrer Wiederherstellungsphrase wiederherstellen.', fr: 'Personne ne peut ouvrir la sauvegarde sans ce mot de passe, pas même nous. Vous pouvez créer un nouveau portefeuille qui remplace la sauvegarde, mais le portefeuille {address} et ses fonds ne pourront alors être récupérés qu’avec sa phrase de récupération.' },
  'signin.startOverAck': { es: 'Entiendo que la copia guardada se reemplaza.', en: 'I understand the stored backup will be replaced.', pt: 'Compreendo que a cópia guardada será substituída.', de: 'Ich verstehe, dass die gespeicherte Sicherung ersetzt wird.', fr: 'Je comprends que la sauvegarde sera remplacée.' },
  'signin.startOverCta': { es: 'Crear una wallet nueva', en: 'Create a new wallet', pt: 'Criar uma wallet nova', de: 'Neue Wallet erstellen', fr: 'Créer un nouveau portefeuille' },
  'signin.protectTitle': { es: 'Proteger la wallet nueva', en: 'Protect the new wallet', pt: 'Proteger a wallet nova', de: 'Neue Wallet schützen', fr: 'Protéger le nouveau portefeuille' },
  'signin.protectDesc': { es: 'Vamos a crear una wallet para {email} en este dispositivo. Ingresá la contraseña de este dispositivo: también protege la copia cifrada que guardamos.', en: 'We will create a wallet for {email} on this device. Enter this device’s password: it also protects the encrypted copy we keep.', pt: 'Vamos criar uma wallet para {email} neste dispositivo. Introduza a palavra-passe deste dispositivo: também protege a cópia cifrada que guardamos.', de: 'Wir erstellen auf diesem Gerät eine Wallet für {email}. Geben Sie das Passwort dieses Geräts ein: Es schützt auch die verschlüsselte Kopie, die wir aufbewahren.', fr: 'Nous allons créer un portefeuille pour {email} sur cet appareil. Saisissez le mot de passe de cet appareil : il protège aussi la copie chiffrée que nous conservons.' },
  'signin.protectReplaceDesc': { es: 'Vamos a crear una wallet nueva para {email}, y su copia reemplaza a la anterior. Ingresá la contraseña de este dispositivo: protege la copia nueva.', en: 'We will create a new wallet for {email}, and its backup replaces the old one. Enter this device’s password: it protects the new backup.', pt: 'Vamos criar uma wallet nova para {email}, e a cópia dela substitui a anterior. Introduza a palavra-passe deste dispositivo: protege a nova cópia.', de: 'Wir erstellen eine neue Wallet für {email}, und ihre Sicherung ersetzt die alte. Geben Sie das Passwort dieses Geräts ein: Es schützt die neue Sicherung.', fr: 'Nous allons créer un nouveau portefeuille pour {email}, et sa sauvegarde remplace l’ancienne. Saisissez le mot de passe de cet appareil : il protège la nouvelle sauvegarde.' },
  'signin.createCta': { es: 'Crear wallet', en: 'Create wallet', pt: 'Criar wallet', de: 'Wallet erstellen', fr: 'Créer le portefeuille' },
  'backup.mismatch': { es: 'La copia guardada no corresponde a la wallet de esta cuenta. No se restauró nada.', en: 'The stored backup does not match this account’s wallet. Nothing was restored.', pt: 'A cópia guardada não corresponde à wallet desta conta. Nada foi recuperado.', de: 'Die gespeicherte Sicherung passt nicht zur Wallet dieses Kontos. Es wurde nichts wiederhergestellt.', fr: 'La sauvegarde ne correspond pas au portefeuille de ce compte. Rien n’a été restauré.' },
  'backup.unreadable': { es: 'No se pudo leer la copia guardada. No se restauró nada.', en: 'The stored backup could not be read. Nothing was restored.', pt: 'Não foi possível ler a cópia guardada. Nada foi recuperado.', de: 'Die gespeicherte Sicherung konnte nicht gelesen werden. Es wurde nichts wiederhergestellt.', fr: 'Impossible de lire la sauvegarde. Rien n’a été restauré.' },
  'backup.wrongPassword': { es: 'Esa no es la contraseña de esta wallet.', en: 'That is not this wallet’s password.', pt: 'Essa não é a palavra-passe desta wallet.', de: 'Das ist nicht das Passwort dieser Wallet.', fr: 'Ce n’est pas le mot de passe de ce portefeuille.' },
  'backup.conflict': { es: 'Mientras tanto esta cuenta guardó una wallet desde otro dispositivo. Volvé a entrar para recuperarla.', en: 'This account backed up a wallet from another device in the meantime. Sign in again to restore it.', pt: 'Entretanto esta conta guardou uma wallet noutro dispositivo. Volte a entrar para a recuperar.', de: 'Dieses Konto hat inzwischen eine Wallet von einem anderen Gerät gesichert. Melden Sie sich erneut an, um sie wiederherzustellen.', fr: 'Ce compte a entre-temps sauvegardé un portefeuille depuis un autre appareil. Reconnectez-vous pour le restaurer.' },
  'backup.staleAfterPwd': { es: 'La copia guardada de {names} todavía se abre con tu contraseña anterior: no se pudo actualizar. Volvé a cambiar la contraseña con conexión para actualizarla.', en: 'The stored backup of {names} still opens with your previous password — it could not be updated. Change the password again while online to update it.', pt: 'A cópia guardada de {names} ainda abre com a palavra-passe anterior: não foi possível atualizá-la. Volte a alterar a palavra-passe com ligação para a atualizar.', de: 'Die gespeicherte Sicherung von {names} öffnet sich weiterhin mit Ihrem vorherigen Passwort – sie konnte nicht aktualisiert werden. Ändern Sie das Passwort erneut, während Sie online sind.', fr: 'La sauvegarde de {names} s’ouvre encore avec votre ancien mot de passe : elle n’a pas pu être mise à jour. Changez à nouveau le mot de passe en ligne pour la mettre à jour.' },

  /* Moving an old Pollar wallet onto a key this device holds (src/lib/pollarMigration.ts). */
  'migrate.title': { es: 'Pasar a tu propia clave', en: 'Move to your own key', pt: 'Passar para a sua própria chave', de: 'Zu Ihrem eigenen Schlüssel wechseln', fr: 'Passer à votre propre clé' },
  'migrate.notPollar': { es: 'Esta wallet ya tiene su clave en este dispositivo. No hay nada que mover.', en: 'This wallet already keeps its key on this device. There is nothing to move.', pt: 'Esta wallet já guarda a chave neste dispositivo. Não há nada a mover.', de: 'Diese Wallet bewahrt ihren Schlüssel bereits auf diesem Gerät auf. Es gibt nichts zu verschieben.', fr: 'Ce portefeuille garde déjà sa clé sur cet appareil. Il n’y a rien à déplacer.' },
  'migrate.intro': { es: 'Esta wallet la custodia Pollar. Vamos a crear una wallet con la clave en este dispositivo y a mover tus fondos a ella. Primero entrá con tu cuenta, así la wallet nueva queda respaldada.', en: 'Pollar custodies this wallet. We will create a wallet whose key lives on this device and move your funds to it. Sign in first, so the new wallet is backed up.', pt: 'Esta wallet é custodiada pela Pollar. Vamos criar uma wallet com a chave neste dispositivo e mover os seus fundos para ela. Inicie sessão primeiro, para que a nova wallet fique com cópia de segurança.', de: 'Diese Wallet wird von Pollar verwahrt. Wir erstellen eine Wallet, deren Schlüssel auf diesem Gerät liegt, und verschieben Ihr Guthaben dorthin. Melden Sie sich zuerst an, damit die neue Wallet gesichert wird.', fr: 'Ce portefeuille est conservé par Pollar. Nous allons créer un portefeuille dont la clé reste sur cet appareil et y déplacer vos fonds. Connectez-vous d’abord, pour que le nouveau portefeuille soit sauvegardé.' },
  'migrate.restoreDesc': { es: '{email} ya tiene una wallet guardada ({address}). Tus fondos se van a mover ahí: ingresá la contraseña con la que la protegiste.', en: '{email} already has a backed-up wallet ({address}). Your funds will move there: enter the password you protected it with.', pt: '{email} já tem uma wallet guardada ({address}). Os seus fundos vão para lá: introduza a palavra-passe com que a protegeu.', de: '{email} hat bereits eine gesicherte Wallet ({address}). Ihr Guthaben wird dorthin verschoben: Geben Sie das Passwort ein, mit dem Sie sie geschützt haben.', fr: '{email} a déjà un portefeuille sauvegardé ({address}). Vos fonds y seront déplacés : saisissez le mot de passe avec lequel vous l’avez protégé.' },
  'migrate.protectDesc': { es: 'Vamos a crear una wallet para {email} en este dispositivo. Ingresá la contraseña de este dispositivo: también protege su copia cifrada.', en: 'We will create a wallet for {email} on this device. Enter this device’s password: it also protects its encrypted backup.', pt: 'Vamos criar uma wallet para {email} neste dispositivo. Introduza a palavra-passe deste dispositivo: também protege a cópia cifrada.', de: 'Wir erstellen auf diesem Gerät eine Wallet für {email}. Geben Sie das Passwort dieses Geräts ein: Es schützt auch ihre verschlüsselte Sicherung.', fr: 'Nous allons créer un portefeuille pour {email} sur cet appareil. Saisissez le mot de passe de cet appareil : il protège aussi sa sauvegarde chiffrée.' },
  'migrate.planIntro': { es: 'Esto es lo que pasa a “{name}”. Pollar firma cada transacción, y la wallet revisa cada una antes de pedirle la firma.', en: 'This is what moves to “{name}”. Pollar signs each transaction, and the wallet checks each one before asking it to.', pt: 'Isto é o que passa para “{name}”. A Pollar assina cada transação, e a wallet verifica cada uma antes de lhe pedir a assinatura.', de: 'Das wird nach „{name}“ verschoben. Pollar signiert jede Transaktion, und die Wallet prüft jede, bevor sie Pollar darum bittet.', fr: 'Voici ce qui est déplacé vers « {name} ». Pollar signe chaque transaction, et le portefeuille vérifie chacune avant de la lui demander.' },
  'migrate.to': { es: 'Destino', en: 'To', pt: 'Destino', de: 'Ziel', fr: 'Destination' },
  'migrate.stays': { es: 'Queda en la cuenta vieja', en: 'Stays in the old account', pt: 'Fica na conta antiga', de: 'Bleibt im alten Konto', fr: 'Reste sur l’ancien compte' },
  'migrate.staysWhy': { es: 'La red exige ese mínimo a toda cuenta abierta, y cerrarla necesita operaciones que la wallet no firma, por tu seguridad.', en: 'The network requires that minimum of every open account, and closing it takes operations the wallet refuses to sign, for your safety.', pt: 'A rede exige esse mínimo a qualquer conta aberta, e fechá-la exige operações que a wallet não assina, por segurança.', de: 'Das Netzwerk verlangt dieses Minimum von jedem offenen Konto, und das Schließen erfordert Operationen, die die Wallet zu Ihrer Sicherheit nicht signiert.', fr: 'Le réseau exige ce minimum de tout compte ouvert, et le fermer demande des opérations que le portefeuille refuse de signer, pour votre sécurité.' },
  'migrate.offers': { es: 'Parte del saldo está reservada en ofertas abiertas y se queda hasta que se cancelen.', en: 'Part of the balance is held by open offers and stays until they are cancelled.', pt: 'Parte do saldo está reservada em ofertas abertas e fica até serem canceladas.', de: 'Ein Teil des Guthabens ist in offenen Angeboten gebunden und bleibt, bis diese storniert werden.', fr: 'Une partie du solde est bloquée dans des offres ouvertes et reste jusqu’à leur annulation.' },
  'migrate.poolShares': { es: 'Las participaciones en pools de liquidez no se mueven con un pago y se quedan.', en: 'Liquidity pool shares cannot move with a payment and stay behind.', pt: 'As quotas em pools de liquidez não se movem com um pagamento e ficam.', de: 'Anteile an Liquiditätspools lassen sich nicht per Zahlung verschieben und bleiben.', fr: 'Les parts de pools de liquidité ne peuvent pas être déplacées par un paiement et restent.' },
  'migrate.insufficientDetail': { es: 'Hacen falta {needed} XLM libres para abrir la wallet nueva y pagar las comisiones; hay {available}. Depositá XLM en esta wallet y volvé a intentar.', en: 'Moving needs {needed} free XLM to open the new wallet and pay the fees; there is {available}. Deposit XLM into this wallet and try again.', pt: 'São precisos {needed} XLM livres para abrir a wallet nova e pagar as comissões; há {available}. Deposite XLM nesta wallet e tente novamente.', de: 'Für den Umzug werden {needed} freie XLM benötigt, um die neue Wallet zu eröffnen und die Gebühren zu zahlen; vorhanden sind {available}. Zahlen Sie XLM auf diese Wallet ein und versuchen Sie es erneut.', fr: 'Le transfert nécessite {needed} XLM disponibles pour ouvrir le nouveau portefeuille et payer les frais ; il y en a {available}. Déposez des XLM sur ce portefeuille et réessayez.' },
  'migrate.insufficient': { es: 'No hay suficientes XLM para mover los fondos.', en: 'There is not enough XLM to move the funds.', pt: 'Não há XLM suficientes para mover os fundos.', de: 'Es ist nicht genug XLM vorhanden, um das Guthaben zu verschieben.', fr: 'Il n’y a pas assez de XLM pour déplacer les fonds.' },
  'migrate.nothing': { es: 'No hay fondos para mover.', en: 'There are no funds to move.', pt: 'Não há fundos para mover.', de: 'Es gibt kein Guthaben zu verschieben.', fr: 'Il n’y a pas de fonds à déplacer.' },
  'migrate.cta': { es: 'Mover fondos', en: 'Move funds', pt: 'Mover fundos', de: 'Guthaben verschieben', fr: 'Déplacer les fonds' },
  'migrate.progress': { es: 'Moviendo… {done} de {total}', en: 'Moving… {done} of {total}', pt: 'A mover… {done} de {total}', de: 'Wird verschoben… {done} von {total}', fr: 'Transfert… {done} sur {total}' },
  'migrate.confirmTitle': { es: 'Mover tus fondos', en: 'Move your funds', pt: 'Mover os seus fundos', de: 'Guthaben verschieben', fr: 'Déplacer vos fonds' },
  'migrate.confirmMsg': { es: 'Todo el saldo de esta wallet pasa a tu wallet nueva, salvo el mínimo que exige la red.', en: 'The whole balance of this wallet moves to your new wallet, except the minimum the network requires.', pt: 'Todo o saldo desta wallet passa para a sua wallet nova, exceto o mínimo exigido pela rede.', de: 'Das gesamte Guthaben dieser Wallet geht auf Ihre neue Wallet über, bis auf das vom Netzwerk verlangte Minimum.', fr: 'Tout le solde de ce portefeuille passe sur votre nouveau portefeuille, sauf le minimum exigé par le réseau.' },
  'migrate.done': { es: 'Tus fondos ya están en tu wallet nueva.', en: 'Your funds are in your new wallet.', pt: 'Os seus fundos já estão na sua wallet nova.', de: 'Ihr Guthaben ist jetzt in Ihrer neuen Wallet.', fr: 'Vos fonds sont sur votre nouveau portefeuille.' },
  'migrate.doneTitle': { es: 'Listo', en: 'Done', pt: 'Concluído', de: 'Fertig', fr: 'Terminé' },
  'migrate.doneDesc': { es: 'Tus fondos están en “{name}”, con la clave en este dispositivo y una copia cifrada que abre tu contraseña.', en: 'Your funds are in “{name}”, with its key on this device and an encrypted backup your password opens.', pt: 'Os seus fundos estão em “{name}”, com a chave neste dispositivo e uma cópia cifrada que a sua palavra-passe abre.', de: 'Ihr Guthaben ist in „{name}“, mit dem Schlüssel auf diesem Gerät und einer verschlüsselten Sicherung, die Ihr Passwort öffnet.', fr: 'Vos fonds sont sur « {name} », avec sa clé sur cet appareil et une sauvegarde chiffrée que votre mot de passe ouvre.' },
  'migrate.leftBehind': { es: 'En la cuenta vieja quedan {amount} XLM como mínimo de la red.', en: '{amount} XLM stays in the old account as the network’s minimum.', pt: 'Ficam {amount} XLM na conta antiga como mínimo da rede.', de: '{amount} XLM bleiben als Netzwerkminimum im alten Konto.', fr: '{amount} XLM restent sur l’ancien compte comme minimum du réseau.' },
  'migrate.openNew': { es: 'Abrir la wallet nueva', en: 'Open the new wallet', pt: 'Abrir a wallet nova', de: 'Neue Wallet öffnen', fr: 'Ouvrir le nouveau portefeuille' },
  'migrate.sourceMissing': { es: 'No se encontró la cuenta de Pollar en la red.', en: 'The Pollar account was not found on the network.', pt: 'A conta Pollar não foi encontrada na rede.', de: 'Das Pollar-Konto wurde im Netzwerk nicht gefunden.', fr: 'Le compte Pollar est introuvable sur le réseau.' },
  'migrate.targetMissing': { es: 'La wallet nueva todavía no existe en la red. Probá de nuevo.', en: 'The new wallet does not exist on the network yet. Try again.', pt: 'A wallet nova ainda não existe na rede. Tente novamente.', de: 'Die neue Wallet existiert noch nicht im Netzwerk. Versuchen Sie es erneut.', fr: 'Le nouveau portefeuille n’existe pas encore sur le réseau. Réessayez.' },
  'migrate.reconnectNeeded': { es: 'Tu sesión de Pollar venció. Volvé a entrar para terminar de mover los fondos.', en: 'Your Pollar session expired. Sign in again to finish moving the funds.', pt: 'A sua sessão Pollar expirou. Volte a entrar para terminar de mover os fundos.', de: 'Ihre Pollar-Sitzung ist abgelaufen. Melden Sie sich erneut an, um das Verschieben abzuschließen.', fr: 'Votre session Pollar a expiré. Reconnectez-vous pour terminer le transfert.' },
  'migrate.reconnectDesc': { es: 'Pollar tiene que firmar una última vez, y tu sesión venció. Entrá de nuevo con la misma cuenta.', en: 'Pollar has to sign one last time, and your session expired. Sign in again with the same account.', pt: 'A Pollar tem de assinar uma última vez e a sua sessão expirou. Entre novamente com a mesma conta.', de: 'Pollar muss ein letztes Mal signieren, und Ihre Sitzung ist abgelaufen. Melden Sie sich erneut mit demselben Konto an.', fr: 'Pollar doit signer une dernière fois et votre session a expiré. Reconnectez-vous avec le même compte.' },
  'migrate.reconnectCta': { es: 'Volver a entrar en Pollar', en: 'Sign in to Pollar again', pt: 'Voltar a entrar na Pollar', de: 'Erneut bei Pollar anmelden', fr: 'Se reconnecter à Pollar' },
  'migrate.reconnected': { es: 'Sesión de Pollar renovada.', en: 'Pollar session renewed.', pt: 'Sessão Pollar renovada.', de: 'Pollar-Sitzung erneuert.', fr: 'Session Pollar renouvelée.' },
  'migrate.reconnectOtherAccount': { es: 'Esa cuenta es otra wallet de Pollar, no esta. Entrá con la cuenta con la que la creaste.', en: 'That account is a different Pollar wallet, not this one. Sign in with the account you created it with.', pt: 'Essa conta é outra wallet Pollar, não esta. Entre com a conta com que a criou.', de: 'Dieses Konto gehört zu einer anderen Pollar-Wallet, nicht zu dieser. Melden Sie sich mit dem Konto an, mit dem Sie sie erstellt haben.', fr: 'Ce compte correspond à un autre portefeuille Pollar. Connectez-vous avec le compte qui a servi à le créer.' },
  'migrate.bannerTitle': { es: 'Tu clave, en tu dispositivo', en: 'Your key, on your device', pt: 'A sua chave, no seu dispositivo', de: 'Ihr Schlüssel, auf Ihrem Gerät', fr: 'Votre clé, sur votre appareil' },
  'migrate.bannerDesc': { es: 'Pollar custodia esta wallet. Pasala a una con la clave en este dispositivo y una copia cifrada que solo abre tu contraseña.', en: 'Pollar custodies this wallet. Move it to one whose key lives on this device, with an encrypted backup only your password opens.', pt: 'A Pollar custodia esta wallet. Passe-a para uma com a chave neste dispositivo e uma cópia cifrada que só a sua palavra-passe abre.', de: 'Pollar verwahrt diese Wallet. Verschieben Sie sie in eine Wallet mit Schlüssel auf diesem Gerät und einer verschlüsselten Sicherung, die nur Ihr Passwort öffnet.', fr: 'Pollar conserve ce portefeuille. Passez à un portefeuille dont la clé reste sur cet appareil, avec une sauvegarde chiffrée que seul votre mot de passe ouvre.' },
  'migrate.bannerCta': { es: 'Empezar', en: 'Start', pt: 'Começar', de: 'Starten', fr: 'Commencer' },
  'migrate.bannerResume': { es: 'Continuar', en: 'Continue', pt: 'Continuar', de: 'Fortsetzen', fr: 'Continuer' },
