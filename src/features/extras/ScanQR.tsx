...
export function ScanQR({ store }: { store: WalletStore }) {
   ...
   * Delegates to `store.applySep7` rather than re-implementing the mapping — this
   * used to keep its own copy, which dropped the memo type and the asset, so a
   * scanned "10 USDC / MEMO_ID" request prefilled 10 XLM with a text memo.
   */
  const applyScan = (text: string): boolean => {
    if (!store.applySep7(text)) return false;
  ...
  const decodeBlob = async (blob: Blob) => {
    try {
      ...
      const scale = Math.min(1, SCAN_DECODE_MAX_PX / Math.max(img.width, img.height));
    ...
    <div className="scr screen col">
      ...
      {devices.length > 1 && !error && (
        <div className="shrink0 scan-device-wrap">
          ...
          {devOpen && (
            <>
              <div onClick={() => setDevOpen(false)} className="scan-dd-backdrop" />
              <div className="scan-dd">
      ...
      <div className="col g8 shrink0 scan-fallbacks">
      ...
      <canvas ref={canvasRef} className="scan-hidden" />
    </div>
  );
}
