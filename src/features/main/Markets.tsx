...
import { Spinner, NavMenu } from '@/ui/parts';
import { buildKind } from '@/lib/platform';
import { MARKET_TOKENS } from '@/constants/markets';
import { MarketRow } from '@/features/main/components/MarketRow';
import { BackCircle } from '@/features/main/shared';
import '@/styles/features/main/shared.css';
import '@/styles/features/main/markets.css';
...
export function Markets({ store }: { store: WalletStore }) {
  ...
  return (
    <div className="scr screen pb-110">
      ...
      {list.map((code, i) => (
        <MarketRow key={code} code={code} price={store.prices[code]} delay={i} onClick={() => { store.setSelectedAsset(code); store.setScreen('asset'); }} />
      ))}
