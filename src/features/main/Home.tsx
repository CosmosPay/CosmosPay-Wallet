...
import { Spinner, NetworkDropdown, NavMenu, SurfaceToggle, EnableReceivingCard } from '@/ui/parts';
import { HistoryRow, GenesisRow } from '@/features/money/Money';
import { HomeAction } from '@/features/main/components/HomeAction';
import { AssetListRow } from '@/features/main/components/AssetListRow';
import { ActivateCard } from '@/features/main/components/ActivateCard';
import { computePortfolio } from '@/lib/portfolio';
import { fmt, splitMoney, pct } from '@/lib/format';
import { getGreeting, ageFromBirthdate } from '@/lib/greeting';
import { useAnimatedNumber } from '@/features/main/shared';
import '@/styles/features/main/home.css';
  ...
  return (
    <div className="scr screen pb-110">
      ...
      {store.history.length === 0 && store.historyLoading ? (
        ...
        <>
          {store.history.slice(0, 10).map((it, i) => <HistoryRow key={it.id} item={it} store={store} delay={i} />)}
          ...
          {store.history.length <= 10 && <GenesisRow store={store} delay={store.history.length} />}
        </>
      )}
    </div>
  );
