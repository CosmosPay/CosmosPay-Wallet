...
import { PrimaryButton, BackBar, Spinner } from '@/ui/parts';
import { Field } from '@/features/fiat/components/Field';
import { Select } from '@/features/fiat/components/Select';
import { PhotoStep } from '@/features/fiat/components/PhotoStep';
    ...
    <>
      ...
      {step === 0 && (
        <>
          ...
          <div className="flexr g10">
            <Select label={t('fiat.country')} value={country} onChange={setCountry} className="fiat-flex">
              {COUNTRIES.map((c) => <option key={c.code} value={c.code}>{c.name}</option>)}
            </Select>
            <Select label={t('fiat.docType')} value={docType} onChange={setDocType} className="fiat-flex">
